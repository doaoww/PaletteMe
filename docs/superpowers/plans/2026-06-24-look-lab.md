...
**Goal:** Build Look Lab — the core paid product that visually transforms the user's face photo to show their correct color season, metals, contrast level, makeup shades, hair color, and hairstyles using MediaPipe CV (client-side) and fal.ai (hairstyles only).

**Architecture:** The AI analysis route generates a `lookLab` data spec (colors, verdicts, explanations) alongside the existing report. A client-side Look Lab component loads MediaPipe once, applies Canvas pixel transforms to the user's stored photo, and renders side-by-side cards with verdicts. Hairstyles are generated once on report unlock via a server-side fal.ai call and cached in Supabase Storage.

**Tech Stack:** `@mediapipe/tasks-vision` (face landmarks + segmentation), Canvas API, `@fal-ai/client` (hairstyles), Zod v4, Next.js App Router, Supabase Storage.
...
- `package.json` — add `@mediapipe/tasks-vision`, `@fal-ai/client`
...
- [ ] **Step 1: Install MediaPipe Tasks Vision and fal.ai client**

```
npm install @mediapipe/tasks-vision @fal-ai/client
```
...
git commit -m "chore: add @mediapipe/tasks-vision and @fal-ai/client"
```
...
**Interfaces:**
- Consumes: `hairstyles` array from `LookLabData`, user photo from request body, Supabase Storage
...
import { LookLabSchema } from "@/lib/look-lab-schema";
import { z } from "zod";
...
async function generateHairstyle(
  ...
  try {
    const result = await fal.run("fal-ai/photomaker", {
      input: {
        image_archive_url: photoDataUrl,
        prompt: `portrait photo, ${styleDescription} hairstyle, natural lighting, photorealistic`,
        style: "Photographic (Default)",
        negative_prompt: "bad quality, blurry, cartoon, unrealistic",
...
export async function POST(req: NextRequest) {
  ...
  const results = await Promise.all(
    hairstyles.map(async style => {
      ...
      return { ...style, generatedImageUrl: imageUrl };
    })
  );

  // Cache to Supabase Storage so subsequent loads are instant
  try {
    ...
    // Non-fatal — client still gets results
  }
...
- [ ] **Step 3: Add `FAL_KEY` note to `.env.local.example` (or `.env.local` if it exists)**
...
git commit -m "feat: hairstyle generation API via fal.ai with Supabase cache"
```
