...
export function scoreColorSeasonCandidates(input: ScoreInput): ScoredSeasonCandidate[] {
  ...
  return SUB_SEASONS.map((profile) => {
    ...
    score += modelHintScore(profile.name, profile.seasonId, input.modelSeason, input.modelSubSeason);
...
function modelHintScore(
  ...
  if (normalizedSubSeason === profileName.toLowerCase()) return 6;
  if (normalizedSubSeason.includes(profileName.toLowerCase())) return 4;
  if (normalizedSeason.includes(profileSeasonId)) return 2;
  return 0;
}
